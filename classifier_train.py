...
#from data.dataset import Target2Dataset
#from data.dataset import Target2Dataset_test
...
#        self.path = data['path']
#        self.weak_label = self.set_device(data['class']) # for batch/plate self.weak_label = None#self.set_device(data['target'])     #plate'])     #'class'])
...
#        print(batch.size())
#        batch_bf = batch[:3] # we don't want to noise the brightfield input
#        batch_cp = batch[3:]
...
#        print("three")
        # Noisy images
        if args.noised:
            t, _ = schedule_sampler.sample(batch_cp.shape[0])#, dist_util.dev())
