...
#        self.classifier.load_state_dict(torch.load("/scratch/kjtm071/openai-2022-12-25-08-28-48-200345/model000250.pt"))#self.args.classifier_path))
...
#        #        classifier.convert_to_fp16()
    ...
    def set_input(self, data):
        ...
        self.path = data['path']
        self.weak_label = self.set_device(data['class']) # 'DMSO' for batch/plate self.weak_label = None#self.set_device(data['target'])     #plate'])     #'class'])
...
#                print('paige!!!')
                ...
                for key, value in self.get_current_visuals(phase='test').items():
                    self.writer.add_images(key, value)
                self.writer.save_images_test(self.save_current_results())
